...
#ifndef RENDERER_MESH_HH
#define RENDERER_MESH_HH
...
auto CreateTriangleMesh(int W, int H, unsigned subdivide) {
    ...
    std::vector<std::array<int, 2>> points{{0, 0},
                                           {W - 1, 0},
                                           {W - 1, H - 1},
                                           {0, H - 1}};
    ...
    for (unsigned n = 0; n < subdivide; ++n) {
        ...
        auto len = [&](int e, auto &b) {
            auto[x0, y0]=points[b[e]];
            auto[x1, y1]=points[b[(e + 1) % 3]];
